...
SCRIPT = ROOT / "goal" / "scripts" / "gemini_goal.py"


def run_goal(tmp_path, *args, session="test-session"):
    env = os.environ.copy()
    env["GEMINI_GOAL_DB"] = str(tmp_path / "goals.sqlite")
    env["GEMINI_GOAL_SESSION_ID"] = session
...
def test_stop_hook_blocks_active_goal(tmp_path):
    ...
    env = os.environ.copy()
    env["GEMINI_GOAL_DB"] = str(tmp_path / "goals.sqlite")
    env["GEMINI_GOAL_SESSION_ID"] = "test-session"
...
def test_cli_does_not_leak_goals_across_sessions(tmp_path):
    ...
    Gemini session into another. The fix is to rely on a stable
    ...
    env = os.environ.copy()
    env["GEMINI_GOAL_DB"] = str(tmp_path / "goals.sqlite")
    env["GEMINI_GOAL_SESSION_ID"] = "session-b"
    result = subprocess.run(
        [sys.executable, str(SCRIPT), "stop-hook"],
...
def test_two_concurrent_terminals_do_not_share_goals(tmp_path):
    """Two Gemini sessions in separate terminal tabs must stay isolated.

    Real-world repro: user runs Gemini in tab A, sets a goal. User opens
    tab B and runs Gemini there with no goal. Tab B's Stop hook used to
    ...
    env_a["GEMINI_GOAL_DB"] = db
    env_a.pop("GEMINI_GOAL_SESSION_ID", None)
    env_a.pop("GEMINI_SESSION_ID", None)
    ...
    env_b["GEMINI_GOAL_DB"] = db
    env_b.pop("GEMINI_GOAL_SESSION_ID", None)
    env_b.pop("GEMINI_SESSION_ID", None)
    ...
    hook_b = subprocess.run(
        [sys.executable, str(SCRIPT), "stop-hook"],
        input=json.dumps({"session_id": "gemini-session-b", "cwd": "/Users/alice/proj-b"}),
        env=env_b, text=True, capture_output=True, check=False,
    )
    ...
    hook_a = subprocess.run(
        [sys.executable, str(SCRIPT), "stop-hook"],
        input=json.dumps({"session_id": "gemini-session-a", "cwd": "/Users/alice/proj-a"}),
        env=env_a, text=True, capture_output=True, check=False,
    )
...
def test_term_session_anchors_goal_across_pwd_drift(tmp_path):
    """A goal set in one Gemini session must remain reachable across cwd drift.
    ...
    env.pop("GEMINI_GOAL_SESSION_ID", None)
    ...
    # Now simulate cwd drift in a Bash subshell of the same Gemini session.
...
def test_stop_hook_finds_goal_via_hook_payload_cwd(tmp_path):
    ...
    hook is given the real Gemini session cwd in its payload, so it
    ...
    # hook payload still carries the real Gemini session cwd.
    env = os.environ.copy()
    env["GEMINI_GOAL_DB"] = str(tmp_path / "goals.sqlite")
    env["GEMINI_GOAL_SESSION_ID"] = "drifted-subshell"
    result = subprocess.run(
        [sys.executable, str(SCRIPT), "stop-hook"],
