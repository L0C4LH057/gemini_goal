---
name: goal
description: Codex-style /goal for Gemini CLI. Use when the user runs /goal, wants a persistent long-running objective, or wants to pause, resume, clear, or complete a goal.
argument-hint: "[status|pause|resume|clear|complete] [--tokens N] <objective>"
---

# Goal

Run the helper first, then obey the returned "Gemini instructions":

```bash
python3 ~/.gemini/skills/goal/scripts/gemini_goal.py invoke "$ARGUMENTS"
```

The helper persists goal state in `~/.gemini/goal/goals.sqlite` and implements the Codex-inspired command surface:

- `/goal <objective>`: set a new active goal for this Gemini session.
...
When a goal is active, continue work toward it instead of merely describing the goal. The installer also adds a Gemini CLI `Stop` hook that prevents stopping while a goal is active, so automatic continuation stops only when the goal is paused, cleared, completed, or the runaway guard is reached.
