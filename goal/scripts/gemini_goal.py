#!/usr/bin/env python3
"""Gemini CLI /goal clone.

The script is intentionally dependency-free: Gemini CLI can execute it from a
...
STATE_DIR = Path(os.environ.get("GEMINI_GOAL_HOME", Path.home() / ".gemini" / "goal"))
DB_PATH = Path(os.environ.get("GEMINI_GOAL_DB", STATE_DIR / "goals.sqlite"))
...
def _term_session_id() -> str | None:
    ...
    is stable for the lifetime of the surrounding Gemini CLI session. That
...
def session_id() -> str:
    ...
    Order of preference:
    - GEMINI_GOAL_SESSION_ID / GEMINI_SESSION_ID (explicit override)
    - TERM_SESSION_ID / ITERM_SESSION_ID (stable across subshells in one
      Gemini CLI session, distinct across separate terminal tabs)
    - PWD-derived hash (last resort; drifts in subshells)
    """
    for key in ("GEMINI_GOAL_SESSION_ID", "GEMINI_SESSION_ID"):
        value = os.environ.get(key)
        if value:
            return value
...
def candidate_session_ids(hook_data: dict[str, Any] | None = None) -> list[str]:
    ...
    slightly different PWD in the same Gemini CLI session. The list
    ...
    sources: list[str | None] = [
        os.environ.get("GEMINI_GOAL_SESSION_ID"),
...
def find_goal(
    ...
    *,
    ...
    leaks goals across separate Gemini sessions: a paused goal in session
...
def set_goal(conn: sqlite3.Connection, sid: str, objective: str, token_budget: int | None) -> sqlite3.Row:
    ...
    if existing:
        raise ValueError("this Gemini session already has a goal; use: /goal clear, then set a new goal")
...
def update_status(conn: sqlite3.Connection, sid: str, status: str) -> sqlite3.Row:
    ...
    if not goal:
        raise ValueError("no goal is set for this Gemini session")
...
def render_goal(row: sqlite3.Row | None) -> str:
    if not row:
        return "No goal is currently set for this Gemini session."
    ...
    if row["token_budget"] is not None:
        parts.append(f"- Token budget: {fmt_tokens(row['token_budget'])} (soft budget; Gemini CLI custom skills do not expose reliable live token counters)")
    return "\n".join(parts)
...
Continue working toward the active Gemini thread goal.
...
`python3 ~/.gemini/skills/goal/scripts/gemini_goal.py complete`
...
def render_invoke_result(action: str, goal: sqlite3.Row | None, extra: str = "") -> str:
    ...
    if goal and goal["status"] == "active":
        ...
        body.extend(["", "Gemini instructions: Do not continue this goal until the user runs `/goal resume`."])
    elif goal and goal["status"] == "budget_limited":
        body.extend(["", "Gemini instructions: The soft budget is exhausted; summarize progress and ask before continuing."])
    return "\n".join(body)
...
def stop_hook() -> int:
    ...
    with sqlite_connect() as conn:
        ...
        max_continues = int(os.environ.get("GEMINI_GOAL_MAX_STOP_CONTINUES", "500"))
        ...
        if recent_count >= max_continues:
            print(
                json.dumps(
                    {
                        "continue": True,
                        "stopReason": f"/goal auto-continuation stopped after {max_continues} Stop-hook continuations. Run /goal resume or raise GEMINI_GOAL_MAX_STOP_CONTINUES to continue automatically.",
                    }
                )
            )
            return 0
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(description="Gemini CLI /goal command")
    sub = parser.add_subparsers(dest="cmd")
    p_invoke = sub.add_parser("invoke", help="Process slash-command arguments and print Gemini-facing instructions")
